...
def check_bipartite(adj_list):
    ...
    while queue:
        curr_node = queue.pop()
        to_assign = 1
        # If the current node has color green adjacent nodes should be assigned
        # blue.
        if node_colors[curr_node] == 1:
            to_assign = 2

        for adj_node in adj_list[curr_node]:
            # If the adjacent node is already colored and if it matches the
            # color of current node then we have an edge between same colored
            # nodes.
            if node_colors[adj_node] != 0 and node_colors[adj_node] != to_assign:
                return False
            else:
    ...
    # We haven't encountered any edge between same same colored nodes.
    return True
...
for edge in range(edges):
    # Edge between which two nodes.(Numbering from 1 to nodes).
