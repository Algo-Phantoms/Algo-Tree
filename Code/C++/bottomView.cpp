/*
 ...
 QUES :  Bottom  view of a binary tree is the set of nodes visible when the tree is viewed from the bottom . Given a binary tree, print the bottom  view of it. The output nodes can be    printed  in any order.

        A node x is there in output if x is the bottomost node at its horizontal distance. Horizontal distance of left child of a node x is equal to horizontal distance of x minus 1, and that of right child is horizontal distance of x plus 1.
*/

/*
 Approach:
      
      1.We will create 2 maps map1 and map2,first we will store the position  corresponding to an node and the other will store the node corresponding to a particular position.
      2.And we will use level order traversal that if it's left/right root exist then it will push in the queue and stores it's pos in the map1. for left node position will become pos-1 and for right node position will become pos+1.
      3.Doing this work till queue does not become empty;
      4.While poping the element ,find the pos corresponding to it .If the key as the pos is already
        present in map2 just update it with the latest node having the the correspondong position .
        place it in map2;
      5.After this whole thing we will have map2 with the unique postions and the node's value corresponding to it and in the ascending order as well.
