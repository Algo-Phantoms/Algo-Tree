...
#include <iostream>
using namespace std;
int searchRotatedArray(int a[], int key, int left, int right)
{
	...
	*/
	...
	{
		if (a[left] <= key and a[mid] > key) //key in present in left part
			return searchRotatedArray(a, key, left, mid - 1);

		return searchRotatedArray(a, key, mid + 1, right);
	}

	if (a[mid] > key and a[right] <= key) //key is present in right part
		return searchRotatedArray(a, key, mid + 1, right);
...
int main()
{
	...
	int index = searchRotatedArray(a, key, 0, n - 1);
	if (index == -1)
	{
