...
*/
ll inverse(ll b,ll m){   // For performing DIVISION (a/b) we have to calculate the multiplicative inverse of 'b' first.
    ll x,y;
    ll g=gcd(b,m,&x,&y);
    if (g != 1) 
        return -1;
...
/* As our inputs are larger (10^50000) we cannot store them using long long int 
or any other datatype, so we are going to store them in string and convert it into
    ...
    while(t--)
    {
        ...
        if(si=="+"){
           ...
           a=modm(x);
            ...
            m-=1;
            /*
            For above,  According to Feramts Little Theorem  
            ...
            Hence mod B with M-1 to reduce the number to a smaller one and then use power() 
            method to compute (a^b)%m. 
