...
Time Complexity: Only one DFS traversal is performed, so the time complexity will be O(V+E) where V is the number of vertices and E is the number of edges in the graph.

Space Complexity: Total vertices of the graph are stored in map, arrays and stack which makes the complexity of the order O(V).
*/